...
const GlobalContextProvider = props => {
  const [themeContext, setThemeContextDispatch] = useReducer(
    (state, actions) => {
      let newState = state;
      newState = {
        ...state,
        mainDarkThemeColor: actions.payload.mainDarkThemeColor,
        mainLightThemeColor: actions.payload.mainLightThemeColor,
        mainThemeColor: actions.payload.mainThemeColor,
        timeZone: actions.payload.timeZone,
      };
  ...
  const [cartContext, cartContextDispatch] = useReducer(
    (state, actions) => {
      let newState = state;
      ...
      switch (actions.type) {
        case 'ADD':
          ...
          let cartData = actions.payload;
          let exist = false;
          cartList.forEach(item => {
            if (
              item?.tableReference.projectId ===
                cartData?.tableReference.projectId &&
              item?.tableReference.datasetId ===
                cartData?.tableReference.datasetId &&
              item?.tableReference.tableId === cartData?.tableReference.tableId
            ) {
              exist = true;
            }
          });
        ...
        default:
          newState = {
            ...state,
            cartList: actions.payload.cartList,
          };
          break;
      }
      return newState;
